...
import { NETFLIX_CANCELLATION, NETFLIX_HOUSE_CHANGE, NETFLIX_OTP, NETFLIX_REQ_RESET_PASSWORD, NETFLIX_TRAVEL_OTP, NETFLIX_VERIFY_EMAIL, DISNEY_OTP } from 'src/constants/email-subject.const';
...
export class EmailForwardProcessorService {
  ...
  async processQueuedPayload(payload: RecieveEmailDto) {
    ...
    try {
          ...
          for (const e of payload.emails) {
            if (e.subject === es.dataValues.subject) {
              ...
              if (data && context) {
                ...
                void this.socketGateway.sendEvent(eventName, {
                  from: e.from,
                  date: e.date,
                  subject: e.subject,
                  data,
                }).catch((error: Error) => {
                  this.logger.error(error.message, error.stack, 'EmailForwardSendEvent');
                });
              }
            }
          }
      ...
      this.logger.error((error as Error).message, (error as Error).stack, 'EmailForwardRecieve');
