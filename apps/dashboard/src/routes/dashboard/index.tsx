  ...
  return (
    <PermissionGate permission="dashboard.view" fallback={
      ...
      <div className="flex flex-col gap-8">
                        ...
                        >
                          ...
                          <CardContent>
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                              Dari
                              {allStatistic.summary.transaction_count}
                              {' '}
                              transaksi
                            </p>
                        ...
                        <Card>
                          ...
                          <CardContent>
                            <div className="text-sm font-medium">
                              {formatDateIdStandard(new Date(allStatistic.meta.start))}
                              {' '}
                              -
                              {formatDateIdStandard(new Date(allStatistic.meta.end))}
                            </div>
                          </CardContent>
                        </Card>
        ...
        {/* Dialog Detail Modal */}
        <Dialog
          open={isCapitalDetailsOpen}
          onOpenChange={(open) => {
            setIsCapitalDetailsOpen(open); if (!open)
              setCapitalPage(1)
          }}
        >
        ...
        <Dialog
          open={isRevenueDetailsOpen}
          onOpenChange={(open) => {
            setIsRevenueDetailsOpen(open); if (!open)
              setRevenuePage(1)
          }}
        >
...
function MonthPicker({ value, onChange }: { value: string, onChange: (val: string) => void }) {
  const defaultYear = value ? Number.parseInt(value.split('-')[0]) : new Date().getFullYear()
...
function YearPicker({ value, onChange }: { value: string, onChange: (val: string) => void }) {
  const currentYear = value ? Number.parseInt(value) : new Date().getFullYear()
