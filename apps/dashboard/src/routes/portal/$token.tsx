...
function PortalPage() {
  ...
  const tenantId = import.meta.env.VITE_TENANT_ID || 'papapremium'
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground animate-pulse">Menghubungkan ke server aman...</p>
        </div>
      </div>
    )
  }
          ...
          <div className="flex items-center gap-3">
            <Card className="bg-white/5 border-white/10 backdrop-blur-md px-4 py-2 flex items-center gap-3">
              <div className="text-right">
                ...
                <p className="text-sm font-mono">{limit?.remaining} / {limit?.total} Sisa</p>
              </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4">
            {messages.length === 0 ? (
              ...
              messages.map((msg) => (
                <Card key={msg.id} className="bg-white/5 border-white/10 hover:bg-white/10 transition-colors group overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-primary opacity-0 group-hover:opacity-100 transition-opacity" />
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <Badge className="bg-primary text-black font-bold uppercase text-[10px]">
                          {msg.parsed_context.replace('NETFLIX_', '').replace('_', ' ')}
                        </Badge>
