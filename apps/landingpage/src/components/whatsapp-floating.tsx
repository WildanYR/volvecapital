      ...
      <AnimatePresence>
        {(isHovered || showTooltip) && (
          <motion.div
            ...
            className="absolute right-0 bg-background text-foreground text-[10px] font-black px-4 py-2 rounded-2xl shadow-[0_15px_30px_rgba(0,0,0,0.15)] border border-border whitespace-nowrap uppercase tracking-widest flex items-center gap-2 pointer-events-none"
            style={{ transform: 'translateZ(0)' }}
          >
            <div className="size-2 bg-[#25D366] rounded-full animate-pulse" />
            Butuh Bantuan?
            <div className="absolute -bottom-1 right-7 w-3 h-3 bg-background rotate-45 border-r border-b border-border" />
          </motion.div>
        )}
      </AnimatePresence>
